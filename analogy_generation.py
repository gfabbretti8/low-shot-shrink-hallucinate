...
def mine_analogies(centroids):
    ...
    for i, c_i in enumerate(centroids):
        ...
        for j, c_j in enumerate(centroids):
            if i==j:
                continue
            print("\r" + str(i) + " " + str(j), end="", flush=True)
...
def train_analogy_regressor(analogies, centroids, base_classes, trained_classifier, lr=0.1, wt=25, niter=8000, step_after=40000, batchsize=128, momentum=0.9, wd=0.0001):
